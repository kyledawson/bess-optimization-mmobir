...
with open('docs/ercot_endpoints.json', 'r') as f:
...
def get_auth_token() -> Optional[str]:
    ...
    if not all([ERCOT_USERNAME, ERCOT_PASSWORD]):
        raise ERCOTAPIError("Missing ERCOT_USERNAME or ERCOT_PASSWORD in environment variables")
...
def get_endpoint_info(category: str, endpoint_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ERCOTAPIError(f"Endpoint {category}/{endpoint_name} not found in configuration")
...
def fetch_day_ahead_prices(
    ...
    Returns:
        Dict containing the day-ahead price data
    """
    if delivery_date is None:
        ...
        try:
            delivery_date_obj = datetime.strptime(delivery_date, "%Y-%m-%d").date()
             ...
             raise ERCOTAPIError("Invalid string format for delivery_date. Use YYYY-MM-DD.")
    else:
